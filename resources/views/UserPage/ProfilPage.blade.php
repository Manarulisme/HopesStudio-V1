...
<div class="flex justify-center items-center min-h-screen pb-16">
    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-md relative text-center">
        ...
        <div class="-mt-12">
            <img
                src="{{ Auth::user()->foto_profil ? asset('storage/foto_profil/' . Auth::user()->foto_profil) : asset('default-profile.png') }}"
                alt="Profile Picture"
                class="w-24 h-24 rounded-full mx-auto border-4 border-white object-cover"
            >
        </div>
        <div class="bg-gray-200 p-4 rounded-lg mt-4 text-left space-y-2">
